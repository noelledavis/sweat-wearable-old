            ...
            if (nextChar != '\0') {
                switch (nextChar) {
                    case 'd':
                        ...
                        break;
                    case 'f':
                        Serial.print(va_arg(argp, double), 2);
                        i += 2;
                        break;
                    case 'h':
                        // Custom extension for printing a double
                        // with no decimal places, NONSTANDARD printf behavior
                        // Currently used to dump sense amp iontophoresis current
                        Serial.print(va_arg(argp, double), 0);
                        i += 2;
                        break;
                    case 's':
...
static inline double compensateCurrent (double rawCurrentReading) {
...
int main (void) {
    ...
    static double channelOneVoltage;
    static double channelTwoVoltage;
    static double currentSenseCurrent;

    static double temporaryChannelOneVoltage;
    static double temporaryChannelTwoVoltage;
    ...
    for (;;) {
        ...
        if (measurementStatusBits == MEASURE_ENABLE_MASK) {
            ...
            // measure sensor channels
            temporaryChannelOneVoltage = readAverage(CH1_ADC_PIN);
            temporaryChannelTwoVoltage = readAverage(CH2_ADC_PIN);

            channelOneVoltage = compensateVoltageCh1(temporaryChannelOneVoltage);
            channelTwoVoltage = compensateVoltageCh2(temporaryChannelTwoVoltage);
            ...
            currentSenseCurrent = currentSenseCurrent / AVERAGING_WINDOW;
